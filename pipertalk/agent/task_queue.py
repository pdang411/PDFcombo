"""Background task queue for longer Piper agent jobs."""
...
class TaskQueue:
    ...
    def submit(
        ...
        speak: Callable | None = None, on_complete: Callable | None = None,
    ) -> str:
        task_id = str(uuid.uuid4())[:8]
        task = Task(
            priority=priority.value, created_at=time.time(),
            task_id=task_id, goal=goal, speak=speak, on_complete=on_complete,
        )
        with self._condition:
    ...
    def get_status(self, task_id: str) -> dict | None:
        with self._lock:
            ...
            return {
                ...
                "status": task.status.value, "result": task.result, "error": task.error,
            }

    def get_all_statuses(self) -> list[dict]:
        with self._lock:
            return [{"task_id": t.task_id, "goal": t.goal[:50], "status": t.status.value} for t in self._tasks.values()]
